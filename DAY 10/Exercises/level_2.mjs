...
const aIntersectionB = new Set(a.filter(num => {
    return b.includes(num)
}))

const aMinusB = new Set(a.filter(num => {
    return !b.includes(num)
}))
console.log(aUnionB)
...
function mostSpokenLanguages(countries_data, limiter) {

    const languageSet = new Set(countries_data.reduce((collectLanguages, country) => {
        if (country.languages.length > 1) {
            country.languages.forEach(language => {
                collectLanguages.push(language)
            })
        }
        else {
            collectLanguages.push(country.languages[0])
        }
    ...
    languageSet.forEach(language => {
        spokenLanguages.push({
            [language]: countries_data.filter(country => {
    ...
    spokenLanguages.sort((languageA, languageB) => {
        const [a, b] = [Object.values(languageA), Object.values(languageB)]
        if (b > a) {
            return -(a - b);
        }
        return b - a;
    })
    if (typeof (limiter) != "number" && limiter == "full") {
        return spokenLanguages;
    }
    if (limiter > spokenLanguages.length) {
        limiter = spokenLanguages.length;
    }
    return spokenLanguages.splice(0, Number(limiter));
}
console.log(mostSpokenLanguages(countries, 150))
